...
export default function Header({ searchSlot, rightSlot }: HeaderProps) {
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-[#E2E8F0] shadow-sm">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 h-14 flex items-center gap-2 sm:gap-3">
        ...
        {searchSlot && (
          <div className="flex-1 min-w-0 max-w-md">{searchSlot}</div>
        )}
        {rightSlot && (
          <div className="flex-shrink-0 ml-auto">{rightSlot}</div>
        )}
      </div>
    </header>
  );
